...
//TODO sorry about mess in this file
const apiUrl: string = process.env.REACT_APP_API_URL ?? "";

export interface SuccessResult<TData = ObjMap<unknown>> {
  data: TData;
}
...
export function grabError(
  ...
  defaultValue: string = "Something wrong",
): string {
  ...
  return fetch(apiUrl, {
    ...
    if (json.errors) {
      responseErrors = json.errors.map(function (error: any) {
        const path: String =
          error.path && error.path.length ? error.path[0] : "";
        if (error.extensions.validation) {
          ...
          return new GQValidationError(error.message, path, items);
        } else {
          return new GQError(error.message, path);
        }
      });
    }
