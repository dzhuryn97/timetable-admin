...
import {
    ...
    EditUserFragmentFragment,
    GetDoctorByIdQuery,
    UpdateDoctorMutationVariables
} from "../../gql/graphql";
import {NotificationType, useDispatchSuccessNotification, useNotificationsDispatch} from "../NotificationShower";
import {FragmentType, graphql, useFragment} from "../../gql";
import {DaySlotTemplateModel, DoctorModel} from "./Models";
...
const updateDoctorMutation = graphql(`
    mutation updateDoctor($id: ID!, $name: String!, $description: String!, $photo: String, $daySlotTemplates: [DaySlotTemplateInput!]!){
        updateDoctor(input: {
            id:  $id,
            name:  $name,
...
function editDoctorToFormValuesTransformer(doctor: EditDoctorFragmentFragment): FormValues {
    ...
    return {
        ...
        photo: doctor.photo,
        daySlotTemplates: doctor.daySlotTemplates.map(function (daySlotTemplate): DaySlotTemplateModel {
...
export function transformDaySlotInputToDaySlotTemplatesValues(daySlotTemplate: EditDoctorFragmentFragment['daySlotTemplates'][0]) {
    return {
...
export default function EditDoctor(props: EditDoctorProps) {
    ...
    async function handleSuccessSubmit(doctor: FormValues): Promise<GQValidationError | void> {
    ...
    return <>
        <DoctorForm doctor={formValues} action={"create"} onSuccessSubmit={handleSuccessSubmit}/>
    </>
