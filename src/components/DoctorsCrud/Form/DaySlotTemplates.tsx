import {Button, FormControl, FormErrorMessage, Input, Select, Table, Tbody, Td, Th, Thead, Tr} from "@chakra-ui/react";
...
export default function DaySlotTemplates({errors, control, register}: DaySlotTemplatesProps) {
    ...
    const daySlotTemplatesNodes = fields.map((daySlotTemplate, index) => {
        ...
        return <Tr key={daySlotTemplate.id}>
            <Td>
                <FormControl isInvalid={!!itemError?.dayNumber}>
                    <Input type={'number'}  {...register(`daySlotTemplates.${index}.dayNumber`,{
                        valueAsNumber: true,
                    ...
                    {itemError?.dayNumber && <FormErrorMessage>{itemError?.dayNumber?.message}</FormErrorMessage>}
