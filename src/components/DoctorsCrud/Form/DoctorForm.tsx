...
import {Box, Button, FormControl, FormErrorMessage, Heading, Input, Textarea} from "@chakra-ui/react";
...
interface DoctorFormProps {
    ...
    onSuccessSubmit: (formValues: FormValues) => Promise<GQValidationError | void>
}
...
export function DoctorForm({doctor, onSuccessSubmit}: DoctorFormProps) {
    ...
    const {register, setError, control, handleSubmit, formState: {errors}} = useForm<FormValues>({
    ...
    return <>
        <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormControl mb="10px" isInvalid={!!errors.photo}>
                ...
                {errors.photo && <FormErrorMessage>{errors.photo.message}</FormErrorMessage>}
            </FormControl>
