...
const UpdateUserMutation = graphql(`
    mutation UpdateUser ($id: ID!, $name: String!, $email: String!, $password: String, $role: UserRoleEnum!) {
        updateUser(input: {
            id: $id,
            name: $name,
            email: $email,
...
function EditUserFragmentToFormInputsTransformer(user: EditUserFragmentFragment): UserFormInputs {
    return {
        name: user.name,
        email: user.email,
        // @ts-ignore
...
function formInputsToMutationTransformer(id: string, userFormInputs: UserFormInputs){
    return {
        id: id,
...
export default function EditUser(props: EditUserProps) {
    ...
    async function UserFormHandler(output: UserFormInputs): Promise<GQError | null> {
        const result = await executeQuery(UpdateUserMutation, formInputsToMutationTransformer(user.id, output))
