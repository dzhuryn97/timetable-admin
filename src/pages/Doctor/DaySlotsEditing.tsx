...
import {Button, Input, Select, Stack, Table, Tbody, Td, Th, Thead, Tr} from "@chakra-ui/react";
import {graphql} from "../../gql";
import {DaySlotEditQueryQuery, DaySlotInput, StatusEnum} from "../../gql/graphql";
...
import MonthSelector, {GetCurrentOption, SelectMonthOption} from "../../components/DoctorsCrud/DaySlots/MonthSelector";
...
import {useDispatchErrorNotification, useDispatchSuccessNotification} from "../../components/NotificationShower";
...
const daySlotQuery = graphql(`
    query DaySlotEditQuery($doctorId: ID!, $month: Int!, $year: Int!, $loadExtra: Boolean! ) {
        doctor(id: $doctorId){
...
const UpdateMonthDaySlotsMutation = graphql(`
    mutation UpdateMonthDaySlots($month: Int!, $year: Int!, $doctorId: ID!, $daySlots: [DaySlotInput!]) {
        updateDaySlots(input: {
            month: $month,
            year: $year,
            doctorId:  $doctorId,
            daySlots: $daySlots
...
export async function loadData(
    doctorId: string, month: SelectMonthOption = GetCurrentOption(), loadExtraData: boolean = true
){
...
function transformResponseToDaySlotValues(inputDaySlots: DaySlotEditQueryQuery['doctor']['daySlots']) {
...
export default function DaySlotsEditing() {
    ...
    const {register,control, getValues, setValue, reset, handleSubmit} = useForm<FormValues>({
        defaultValues: {
    ...
    async function handleSelectMonth(month: SelectMonthOption) {
        ...
        if(response.data.doctor){
            reset({
                'daySlots': transformResponseToDaySlotValues(response.data.doctor.daySlots)
            })
        }
    ...
    const onSubmit: SubmitHandler<FormValues> =async (data) => {
       ...
       const response =  await executeQuery(UpdateMonthDaySlotsMutation, {
            ...
            doctorId: doctorId,
            daySlots: data.daySlots.map(function (daySlot: DaySlotValues): DaySlotInput{
                return {
    ...
    function handleSave(){
            ...
            // executeQuery(UpdateMonthDaySlotsMutation, {
            //     month: selectedMonth.month,
            //     year: selectedMonth.year,
            //     doctorId: doctorId,
            //     daySlots: daySlots.map(function (daySlot: DaySlotValues): DaySlotInput{
            //         return {
            //             date: daySlot.date,
            //             status: daySlot.status,
            //             workHours: daySlot.workHours,
            ...
            //     })
            // })
            // notificationDispatcher({type: 'add', message: 'Updater', messageType: NotificationType.SUCCESS})
            // notificationDispatcher({type: 'add', message: 'Error', messageType: NotificationType.ERROR})
    ...
    const daySlotNodes = fields.map(function (daySlot, index) {
        ...
        function insertDaySlot(index: number) {
            ...
            const nextDate = new Date(prevDate.getFullYear(),prevDate.getMonth(),prevDate.getDate()+1)
        ...
        return <Tr key={daySlot.id}>
            <Td>
                <SingleDatepickerControlled<FormValues>  control={control} name={`daySlots.${index}.date`} />
            </Td>
            <Td>
            ...
            <Td>
                {daySlot.status === StatusEnum.Present &&
                    <Input placeholder={'Work hours'} {...register(`daySlots.${index}.workHours`)}/>
                }

                {daySlot.status === StatusEnum.Absent &&
                    <Input placeholder={'Absent reason'} {...register(`daySlots.${index}.absentReason`)} />
                }
            </Td>
            <Td>
                {daySlot.status === StatusEnum.Absent &&
